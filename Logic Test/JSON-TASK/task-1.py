...
# Define the start and end dates for March
start_date = datetime.datetime(2018, 3, 1)
end_date = datetime.datetime(2018, 3, 31, 23, 59, 59)

# Filter the purchases to include only those in March
march_purchases = [p for p in purchases if start_date <= datetime.datetime.fromisoformat(p['created_at']) <= end_date]

# Print the results
print(march_purchases)